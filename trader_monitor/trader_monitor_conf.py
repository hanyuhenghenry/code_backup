...
def cal_sdate(date):
   l = calendar.index[calendar.index<=date]
   sdate = l[-3]
   # print(sdate)
   return sdate


def cal_percentile(df, factor, k):
   return np.percentile(df[factor], k)

def pro_data(symbol, date):
   #if symbol not in list_live:
   #   return {}
   print(symbol)
   cols = ['low_opi/vol','high_opi/vol','big_volume','above_prev_ask','below_prev_bid']
   df = pd.read_csv('/home/public/henryhan/tick_level_factors/min/'+symbol, sep='\t', usecols=['timestamp_exchange'])
   df = pd.read_csv('/home/public/henryhan/tick_level_factors/min/'+symbol, sep='\t', skiprows=lambda x: x>0 and x<=df.shape[0]-5000)
   sdate = cal_sdate(date)
   # print(date, sdate)
   df = df[(df.date>=sdate) & (df.date<=date)]
   # print(df)
   df = df.sort_values('timestamp_exchange')
   # df['price_hl'] = df['above_prev_ask'] - df['below_prev_bid']
   # df['winrate_hl'] = df['up_ticks_pct'] - df['down_ticks_pct']
   # df['opi_hl'] = df['big_positive_opi'] - df['big_negative_opi']
   # df['big_volume_raw'] = df['big_volume']
   # df['mom'] = df['close'].diff(n)
   # df['mom1'] = df['mom']
   # df['mom_before'] = df['close'].diff(n).shift(n)
   # df['hour'] = df['time'].str.slice(0,2).astype(int)
   # df['session'] = (df['hour']>8)*(df['hour']<16)*1

   # 计算各个因子的thresh
   thx_dict = dict()
   for factor in cols:
      thx_dict[factor+'_thresh'] = float(cal_percentile(df,factor, 99.5))

   thx_dict['tick_vol_mean'] = float(df['latest_vmean'].values[-1])
   thx_dict['tick_vol_99cut'] = float(df['latest_vcut'].values[-1])
   return thx_dict
...
def get_strategy_info_longterm(strategy, symbol, df_convert_symbol, df_sha_symbol, date, session):
   ...
   if symbol in symbol_lists['zce']:
      ...
      ex = 'shfe'
   model_info['path']=['/shared/%s/%s/%d'%(ex, bsess, bdate)]
...
#  model_info['twap_gap_time'] = 100
   ...
   thx_date = int(calendar.loc[date, 'prev_date']) if ARGS.session == 'day' and ARGS.backtrade else date
   thx_dict = pro_data(symbol, thx_date)
   for k, v in thx_dict.items():
      model_info[k] = v
   return model_info
...
if __name__=="__main__":
   ...
   PARSER.add_argument('-date','--date',help='date', type=int, default=last_date)
   ...
   PARSER.add_argument('-bdate', help='backtrade_date', type=int, default=0)
   PARSER.add_argument('-bsess', help='backtrade_session', default='')
   ...
   bdate = ARGS.date if not ARGS.bdate else ARGS.bdate
   bsess = ARGS.session if not ARGS.bsess else ARGS.bsess
   ...
   strategy_exe_file = '/home/public/monitor/trader_monitor/strategy_conf.json'
